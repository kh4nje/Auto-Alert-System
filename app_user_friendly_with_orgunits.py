...
def check_and_install_openpyxl():
    if not importlib.util.find_spec("openpyxl"):
        with st.spinner("Installing required library (openpyxl)... This may take a moment.")
            try:
                subprocess.check_call([sys.executable, "-m", "pip", "install", "openpyxl"])
                st.success("✅ Successfully installed openpyxl!")
            except subprocess.CalledProcessError:
                st.error("❌ Failed to install openpyxl. Please try running the app again or contact support.")
                st.stop()

check_and_install_openpyxl()
